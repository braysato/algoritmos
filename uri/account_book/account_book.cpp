...
int main() {
    ...
    while (cin >> N >> F && (N != 0 || F != 0)) {
        ...
        // Verificar si F es alcanzable
        if (F > total_sum || F < -total_sum || (total_sum + F) % 2 != 0) {
            cout << "*\n";
            continue;
        }
        
        // Usar meet-in-the-middle para manejar N hasta 40
        string result(N, '?');
        
        // Para N pequeño, usar fuerza bruta optimizada
        if (N <= 20) {
            ...
            // Determinar qué transacciones son fijas
            for (int i = 0; i < N; ++i) {
            ...
            // Para N > 20, usar meet-in-the-middle
            int half = N / 2;
